...
module.exports = (task, core) => {
  ...
  let parser = {
    ...
    node: function(page, node) {
      ...
      if (this.pages[page][node]['mix'].length > 0) {
        this.pages[page][node]['mix'].forEach(mixNode =>
          this.node(page, mixNode)
        );
      }
    ...
    attr: function(attr) {
      if (
        ...
        attr.indexOf('data:') === 0 ||
        /^(?:https?\:)?\/\//i.test(attr) ||
        attr.indexOf(core.config.dist.static + core.path.SEP) === -1
      )
        return;
      ...
      attr = attr
        .replace(block, block + core.path.SEP + 'assets')
        .replace(/ 2x| 3x| 4x/g, '');
    ...
    let newParser = new htmlparser.Parser({
      onopentag: function(tag, attrs) {
        if (tag === 'script') {
          ...
          if (file !== 'app.js' && core.used.scripts.indexOf(file) === -1)
            core.used.scripts.push(file);
        ...
        if (tag === 'link') {
          if (
            ...
            /^(?:https?\:)?\/\//i.test(attrs.href)
          )
            return;

          let file = core.getBasename(attrs.href, true);

          if (file !== 'app.css' && core.used.styles.indexOf(file) === -1)
            core.used.styles.push(file);
        ...
        if (tag === 'use') {
          ...
          if (core.used.symbol.indexOf(file) === -1)
            core.used.symbol.push(file);
        ...
        classes.forEach((cls, i) => {
          ...
          if (core.bem.isBlock(cls) && core.used.blocks.indexOf(cls) === -1)
            core.used.blocks.push(cls);
          ...
          if (!parser.pages[name][val])
            parser.pages[name][val] = { mod: [], mix: [] };
          if (!parser.pages['app'][val])
            parser.pages['app'][val] = { mod: [], mix: [] };
  ...
  if (
    !core.config.blocks.app.symbol &&
    (core.isDevelopment || core.used.symbol.length > 0)
  )
    core.config.blocks.app.symbol = '';
  ...
  Object.keys(core.config.blocks).forEach(page => {
    ...
    Object.keys(core.config.blocks[page]).forEach(node => {
      let array = Array.isArray(core.config.blocks[page][node])
        ? core.config.blocks[page][node]
        : [
            typeof core.config.blocks[page][node] === 'string'
              ? core.config.blocks[page][node]
              : ''
          ];
      ...
      if (!parser.parsed[page][node]) {
        ...
        array.forEach(key => {
          if (parser.parsed[page][node].indexOf(key) === -1)
            parser.parsed[page][node].push(key);
        });
      }
    });
  });
  ...
  let usedStyles = Array.isArray(core.config.use.styles)
    ? core.config.use.styles
    : [core.config.use.styles];

  usedStyles.forEach(style => {
    if (typeof style !== 'string') return;

    if (style !== 'app.css' && core.used.styles.indexOf(style) === -1)
      core.used.styles.push(style);
  });

  // Add scripts from config

  let usedScripts = Array.isArray(core.config.use.scripts)
    ? core.config.use.scripts
    : [core.config.use.scripts];

  usedScripts.forEach(script => {
    if (typeof script !== 'string') return;

    if (script !== 'app.js' && core.used.scripts.indexOf(script) === -1)
      core.used.scripts.push(script);
  });

  // Add symbol from config

  let usedSymbol = Array.isArray(core.config.use.symbol)
    ? core.config.use.symbol
    : [core.config.use.symbol];
  ...
  let usedAssets = Array.isArray(core.config.use.assets)
    ? core.config.use.assets
    : [core.config.use.assets];
  ...
  usedAssets.forEach(item => {
    ...
    let file,
      block = item.split(core.path.SEP)[0];
  ...
  if (core.used.styles.length > 0)
    core.used.assets.push(
      core.path.blocks(`*/*/assets/{${core.used.styles.join(',')}}`)
    );

  if (core.used.scripts.length > 0)
    core.used.assets.push(
  ...
  if (core.config.autoCreate) {
    Object.keys(core.tree.app).forEach(node => {
      ...
      if (levels.length > 0) {
        ...
        levels.forEach(level => {
          checkNodes.forEach(el => {
            ...
            Object.keys(add).forEach(key => {
              ...
              if (core.checkFile(file) && ignore[key].indexOf(el) === -1)
                ignore[key].push(el);
            });
          });
        });
        ...
        Object.keys(add).forEach(key => {
          add[key] = [].concat(
            checkNodes.filter(val => ignore[key].indexOf(val) < 0)
          );
        });
      }

      Object.keys(add).forEach(key => {
        if (core.config.autoCreateAdd.indexOf(key) !== -1)
          core.addFile(add[key], key);
      });
    });
  }
