...
module.exports = (task, core) => {
  let imports = {},
    helper = 'global' + core.config.extnames.styles,
  ...
  Object.keys(core.tree).forEach(page => {
    if (
      (!core.config.options.cssBundles || core.isDevelopment) &&
      page !== 'app'
    )
      return;
    ...
    Object.keys(core.tree[page]).forEach(key => {
      ...
      core.levels.forEach(level => {
        let el = core.path.join(
            level,
            block,
            key + core.config.extnames.styles
          ),
        ...
        if (
          core.config.extnames.styles !== '.css' &&
          core.checkFile(core.path.blocks(css))
        ) {
        ...
        core.tree[page][key].forEach(mod => {
          let elMod = core.path.join(
              level,
              block,
              mod + core.config.extnames.styles
            ),
          ...
          if (
            core.config.extnames.styles !== '.css' &&
            core.checkFile(core.path.blocks(cssMod))
          ) {
  ...
  helper = core.checkFile(core.path.blocks(helper))
    ? `@import '${core.path.join(relative, helper)}';`
    : false;
