...
module.exports = (task, core) => {
  ...
  Object.keys(core.tree).forEach(page => {
    if (
      (!core.config.options.jsBundles || core.isDevelopment) &&
      page !== 'app'
    )
      return;
    ...
    Object.keys(core.tree[page]).forEach(key => {
      ...
      core.levels.forEach(level => {
        let el = core.path.join(
          level,
          block,
          key + core.config.extnames.scripts
        );
        ...
        core.tree[page][key].forEach(mod => {
          let elMod = core.path.join(
            level,
            block,
            mod + core.config.extnames.scripts
          );
  ...
  Object.keys(imports).forEach(page => {
    let content = '',
      file = core.path.temp(page + core.config.extnames.scripts);

    imports[page].forEach(
      key =>
        (content += `${content === '' ? '' : '\n'}require('${key.replace(
          /\\/g,
          '/'
